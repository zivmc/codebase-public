...
protected:
    ...
    enum {
        OBJECT_LIFETIME_STRONG  = 0x0000, // 强引用计数为 0 时就释放对象
        OBJECT_LIFETIME_WEAK    = 0x0001, // 强引用计数、弱引用计数都为 0 时释放对象
...
private:
    ...
    // incWeak, decWeak 实际调用的是这里 mRefs 的成员函数
...
public:
    ...
    // 相较于 sp，少了两个移动构造函数，多了两个使用 sp 类型的构造函数
...
template<typename T>
// 如果构造函数的参数类型是 RefBase*，由于 wp 不是 RefBase 的友元类，
// 不能访问其 mRefs，只能通过 RefBase 提供的 createWeak 接口增加弱引用计数
wp<T>::wp(T* other)
    : m_ptr(other)
{
...
template<typename T>
// other 中已经有了 RefBase 类的 mRefs 成员，可以直接调用其 incWeak 函数增加弱引用计数
...
// 构造函数：使用 wp 对象初始化时 m_refs 通过 incWeak 初始化；
// 其他情况下使用 createWeak 初始化
